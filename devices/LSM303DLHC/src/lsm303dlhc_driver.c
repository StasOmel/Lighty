...
* Return		: None
*******************************************************************************/
static u8_t ReadReg(u8_t deviceAddr, u8_t Reg, u8_t* Data) {
...
* Return		: None
*******************************************************************************/
static u8_t WriteReg(u8_t deviceAddress, u8_t WriteAddr, u8_t Data) {
...
//  u8_t raw[6];
//
//  I2C_BufferRead(raw, MAG_I2C_ADDRESS, OUT_X_L_M, 6);
//
//  valueL = raw[0];
//  valueH = raw[1];
//  buff->AXIS_X = (i16_t)( (valueH << 8) | valueL );
...
//stml
status_t DummyReadReferenceReg(void)
{
	u8_t value;
  if( !ReadReg(ACC_I2C_ADDRESS, REFERENCE_REG_A, &value) )
	return MEMS_ERROR;
